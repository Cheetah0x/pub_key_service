...
export function blockNumberToNoirNonce(
  blockNumber: string | number | bigint
): number[] {
  console.log("blockNumberToNoirNonce", blockNumber);
  const blockStr =
    typeof blockNumber === "string"
      ? blockNumber.replace("0x", "")
      : blockNumber.toString();
  console.log("length", blockStr.length);
  const nonceBytes = Array.from(Bytes.fromString(blockStr));
  return utils.arrayPadEnd(nonceBytes, JWT_NONCE_LEN, 0);
}
